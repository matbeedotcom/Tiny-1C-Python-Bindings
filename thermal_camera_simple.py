...
try:
    ...
    thermal_camera = None
    print("Warning: thermal_camera module not found. Please build first with:")
...
class SimpleThermalCamera:
    ...
    def __init__(self):
        if thermal_camera is None:
            raise RuntimeError("thermal_camera module not available")
        
        self._camera = thermal_camera.ThermalCamera()
        self._temp_processor = thermal_camera.TemperatureProcessor()
    ...
    def get_temperature_celsius(self, temp_frame: np.ndarray) -> np.ndarray:
        ...
        """
        return np.vectorize(thermal_camera.temp_to_celsius)(temp_frame)
